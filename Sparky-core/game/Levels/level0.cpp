...
void Level0::RenderScene()
{
	this->scenes[indexOfCurrScene]->updateCamera((static_cast<Transform*>(player->getComponent(ComponentIDEnum::TransformComp))->location));
	this->scenes[indexOfCurrScene]->renderLayers();
}

void Level0::AddRenderableToCurrScene(Renderable2D* renderable)
{
	this->scenes[indexOfCurrScene]->objectLayer->add(renderable);
}